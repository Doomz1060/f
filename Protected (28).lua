...
ThemeManager:SetFolder('DoomzWareFrontLine1-cheat')
SaveManager:SetFolder('DoomzWareFrontLine1-cheat')
...
do
	...
	--	friendly_params.FilterDescendantsInstances = Toggles.Wallbang.Value and { workspace:findFirstChild('workspace') }
		--enemy_params.FilterDescendantsInstances = Toggles.Wallbang.Value and { workspace:findFirstChild('workspace') }
			end
	...
	if spawn_bullet then
		...
		enemy_params = getupvalue(spawn_bullet, 8)
	end
end

local window = UI:CreateWindow('DoomzWare') do
	...
	--	gAimbot:AddToggle('Wallbang', { Text = 'Wallbang' })
		...
		gAimbot:AddSlider('CircleRadius', { Text = 'Circle radius', Min = 20, Max = 1000, Default = 60, Rounding = 0 })
		...
		gVisuals:AddDropdown('ESPFlags', { Text = 'ESP', Multi = true, Values = { 'Names', 'Boxes', 'Tracers', } })
	...
	do
		...
		tWeapons:AddSlider('FastFireMult', { Text = 'Fire rate multiplier', Min = 0, Max = 200, Rounding = 1, Default = 1 })

		tWeapons:AddToggle('BulletVelocity', { Text = 'Increased bullet velocity' })
		tWeapons:AddSlider('BulletVelocityMult', { Text = 'Velocity multiplier', Min = 0, Max = 200, Rounding = 1, Default = 1 })
	end
	...
	local tSettings = window:AddTab('UI Settings') do
		...
		tCredits:AddLabel('Doomz - Script')
		tCredits:AddLabel('Wally - UI library')
		tCredits:AddLabel('Dante - ESP library')

		tCredits:AddButton('Copy discord invite', function()
			if pcall(setclipboard, 'https://discord.gg/VHHW8VcN6J') then
